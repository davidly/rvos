...
set _testlist=t glob ts sieve e tap tpi tphi tins terrno tp ttt tf^
              tm ttime td fileops t_setjmp tex trw empty tmuldiv
...
%_rvoscmd% -h:30 debianrv\rvos -p debianrv\tp.elf 1 >>%outputfile%

echo running ntvao, ntvcm, and ntvdm >>%outputfile%
%_rvoscmd% -h:30 debianrv\ntvao -c -p ../ntvao/ttt1.hex >>%outputfile%
%_rvoscmd% -h:30 debianrv\ntvcm -c -p ../ntvcm/tttcpm.com 1 >>%outputfile%
%_rvoscmd% -h:30 debianrv\ntvdm -c -p ../ntvdm/ttt8086.com 1 >>%outputfile%
