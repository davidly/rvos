...
* Notes:
    ...
    * Does not work with apps generated by Go, whose runtime apparently requires mmap to work to initialize the memory allocator.
    * Works with rust apps provided they're statically linked, e.g.: rustc -O -C target-feature=+crt-static sample.rs
    * Rust app startup performance is slow due to extensive FDES unwind parsing by the rust runtime.
